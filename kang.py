...
async def _(event):
    ...
    packname = f"ᴠɪʟʟᴀɪɴ @Villain_Of_Telegram"
    packshortname = f"ᴠɪʟʟᴀɪɴ{userid}"  # format: Uni_Borg_userid

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "ᴠɪʟʟᴀɪɴ_Sticker.png"
    file = await borg.download_file(reply_message.media)
    uploaded_sticker = None
    if is_a_s:
        file_ext_ns_ion = "AnimatedSticker.tgs"
        uploaded_sticker = await borg.upload_file(file, file_name=file_ext_ns_ion)
        packname = f"ᴠɪʟʟᴀɪɴ_AnimatedStickers"
        packshortname = f"ᴠɪʟʟᴀɪɴ_Animated"  # format: Uni_Borg_userid
